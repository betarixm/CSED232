...
int greedy_max(std::vector<int>& students); // greedy하게 리코더를 가져갈 수 있는 최대의 학생 수를 구하는 함수
...
int main(){
    ...
    while(loss_num-- && cin >> idx){ // 잃어버린 학생 수 만큼 반복
        students[idx - 1] -= 1; // 학생 벡터에서 잃어버린 학생 번호를 참조하여 1을 뺀다.
    }

    while(extra_num-- && cin >> idx){ // 여분이 있는 학생 수 만큼 반복
        students[idx - 1] += 1; // 학생 벡터에서 여분이 있는 학생 번호를 참조하여 1을 더한다.
    }

    result_num = greedy_max(students); // 리코더를 가져갈 수 있는 최대 학생 수를 구하는 함수 greedy_max 호출.
...
int greedy_max(std::vector<int>& students){ // greedy하게 리코더를 가져갈 수 있는 최대의 학생 수를 구하는 함수
    ...
    for(idx = 0; idx < students.size(); idx++){ // 학생 벡터 iterate.
        if(students[idx] < 0) { // 어떤 학생의 값이 0보다 작다면, 즉 리코더를 도난당했다면,
            if (idx - 1 >= 0 && students[idx - 1] > 0) { // 그런데 앞 번호 친구에게 여분의 리코더가 있다면,
                students[idx - 1] -= 1; // 여분의 리코더를
                students[idx] += 1; // 도난 당한 친구에게 대여
            } else if (idx + 1 < students.size() && students[idx + 1] > 0) { // 앞 번호 친구에게는 없고, 뒷번호 친구에게 있다면,
                students[idx + 1] -= 1; // 여분의 리코더를
                students[idx] += 1; // 도난 당한 친구에게 대여
            }
        }
